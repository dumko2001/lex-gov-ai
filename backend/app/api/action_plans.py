...
from app.models.all import ActionPlan, Directive, VerificationAction, FeedbackContext, ProcessingJob, Judgment
from app.schemas.__init__ import (
    ActionPlanResponse, VerificationActionRequest, VerificationActionResponse,
    DirectiveEditRequest, DirectiveRedoRequest
)
...
def get_action_plan(action_plan_id: str, db: Session = Depends(get_db)):
    ...
    directives = db.query(Directive).filter(Directive.action_plan_id == action_plan_id).all()
    ...
    response = ActionPlanResponse(
        ...
        overall_confidence=float(plan.overall_confidence) if plan.overall_confidence else None,
        status=plan.status,
...
@router.post("/{action_plan_id}/directives/{directive_id}/verify", response_model=VerificationActionResponse)
def verify_directive(
    ...
    directive = db.query(Directive).filter(
        ...
        Directive.action_plan_id == action_plan_id
    ).first()
    ...
    original_value = {
        ...
        "deadline_explicit": str(directive.deadline_explicit) if directive.deadline_explicit else None,
        "deadline_inferred": str(directive.deadline_inferred) if directive.deadline_inferred else None,
    }
    ...
    if request.action_type == "APPROVE":
        ...
        directive.verified_at = __import__('datetime').datetime.now(__import__('datetime').timezone.utc)
        ...
        if "deadline_explicit" in request.edited_value:
            ...
            directive.deadline_explicit = datetime.strptime(request.edited_value["deadline_explicit"], "%Y-%m-%d") if request.edited_value["deadline_explicit"] else None
        if "deadline_inferred" in request.edited_value:
            from datetime import datetime
            directive.deadline_inferred = datetime.strptime(request.edited_value["deadline_inferred"], "%Y-%m-%d") if request.edited_value["deadline_inferred"] else None
    ...
    action = VerificationAction(
        ...
        officer_id=directive.verified_by or __import__('uuid').uuid4(),  # Placeholder - should be from auth
...
def publish_action_plan(action_plan_id: str, db: Session = Depends(get_db)):
    ...
    unverified = db.query(Directive).filter(
        Directive.action_plan_id == action_plan_id,
        Directive.status != "VERIFIED"
    ).count()
    
    if unverified > 0:
        raise HTTPException(400, f"Cannot publish: {unverified} directive(s) still unverified")
