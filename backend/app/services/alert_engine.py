    ...
    def generate_alerts(self, db: Session, judgment_id: str) -> list:
        ...
        for directive in directives:
            ...
            if directive.deadline_inferred:
                ...
                elif days_remaining <= 10 and days_remaining >= 0 and directive.status == "UNVERIFIED":
                    alerts.append(Alert(
                        judgment_id=judgment_id,
                        directive_id=directive.id,
            ...
            if directive.action_type == "CONSIDER_APPEAL" and directive.deadline_inferred:
                days_remaining = _days_until(directive.deadline_inferred)
                if days_remaining <= 15 and days_remaining >= 0 and directive.status == "UNVERIFIED":
                    alerts.append(Alert(
                        judgment_id=judgment_id,
                        directive_id=directive.id,
