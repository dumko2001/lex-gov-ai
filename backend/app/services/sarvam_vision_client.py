...
class SarvamVisionClient:
    ...
    async def _process_multiple_chunks(self, file_path: str, total_pages: int, language: str) -> str:
        ...
        for _ in range(max_retries):
            async with httpx.AsyncClient() as client:
                ...
                if status in ["COMPLETED", "SUCCESS"]:
                    ...
                    raise ValueError(f"Job {job_id} failed: {data.get('error_message', 'Unknown error')}")
                
                await asyncio.sleep(2)
        
        raise TimeoutError(f"Job {job_id} did not complete within {max_retries * 2} seconds")
